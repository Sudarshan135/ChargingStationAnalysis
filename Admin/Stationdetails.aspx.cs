...
public partial class Admin_Stationdetails : System.Web.UI.Page
{
    ...
    {
        if (Request.QueryString["status"].ToString() == "Active")
            btnStatus.Text = "Deactive";
        else
        ...
        txtsID.Text = Request.QueryString["stationID"].ToString();

        con = new SqlConnection(ConfigurationManager.AppSettings["LIS"]);
        cmd = new SqlCommand();
        con.Open();
        cmd.CommandText = "select dayTime,Pspaces,Paymodes,unitcost,state,Area,Pincode,vehicleTypes,rapidcharg,fastCharger,slowcharger,loc1,loc2,loc3,loc4,loc5,loc6 from stationDetails where stationID=@stationID";
        cmd.Parameters.AddWithValue("@stationID", Request.QueryString["stationID"].ToString());
    ...
    protected void btnsave_Click(object sender, EventArgs e)
    {
        ...
        con.Open();
        cmd.Connection = con;
        cmd.CommandText = "insert into stationDetails (sID,stationID,dayTime,Pspaces,Paymodes,unitcost,state,Area,Pincode,vehicleTypes,rapidcharg,fastCharger,slowcharger,loc1,loc2,loc3,loc4,loc5,loc6) values(@sID,@stationID,@dayTime,@Pspaces,@Paymodes,@unitcost,@state,@Area,@Pincode,@vehicleTypes,@rapidcharg,@fastCharger,@slowcharger,@loc1,@loc2,@loc3,@loc4,@loc5,@loc6)";
        ...
        cmd.Parameters.AddWithValue("@Area", txtcity.Text);
        ...
        if (n > 0)
        {
            ...
            ScriptManager.RegisterStartupScript(this, GetType(), "station_SbookingStatus", "alert('" + message + "');", true);
        }
        con.Close();
    ...
    {
        if (btnStatus.Text == "Deactive")
        {
            SqlConnection con = new SqlConnection(ConfigurationManager.AppSettings["LIS"]);
            SqlCommand cmd = new SqlCommand();
            con.Open();
            cmd.Connection = con;
            cmd.CommandText = "Update StationRegistration set status='Deactive' where StationID=@StationID";


            cmd.Parameters.AddWithValue("@StationID", Convert.ToInt32(Request.QueryString["stationID"].ToString()));
            cmd.ExecuteNonQuery();
            con.Close();

            string message = "Station Deactivated";
            ScriptManager.RegisterStartupScript(this, GetType(), "station_SbookingStatus", "alert('" + message + "');window.location='StationList.aspx';", true);
        ...
        else
        {
            ...
            con.Open();
            cmd.Connection = con;
            cmd.CommandText = "Update StationRegistration set status='Active' where StationID=@StationID";
            ...
            string message = "Station Activated";
            ScriptManager.RegisterStartupScript(this, GetType(), "station_SbookingStatus", "alert('" + message + "');window.location='StationList.aspx';", true);
